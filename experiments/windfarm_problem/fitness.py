...
def fitness_np(population: np.ndarray) -> np.ndarray:
    ...
    tiled_x = np.tile(population[:, 0, np.newaxis], (1, cos_dir.shape[-1], 1)).transpose(0, 2, 1)
    tiled_y = np.tile(population[:, 1, np.newaxis], (1, sin_dir.shape[-1], 1)).transpose(0, 2, 1)
    ...
    turb_pwr = np.zeros_like(np.transpose(_x, (0, 2, 1)))
...
def fitness_tf(population: tf.Tensor) -> tf.Tensor:
    ...
    tiled_x = tf.tile(tf.expand_dims(population[:, 0], axis=-1), (1, 1, cos_dir.shape[-1]))
    tiled_y = tf.tile(tf.expand_dims(population[:, 1], axis=-1), (1, 1, sin_dir.shape[-1]))
    ...
    turb_pwr = tf.zeros_like(tf.transpose(_x, perm=[0, 2, 1]))

    cut_in_mask = (this_turb_ci <= wind_speed_eff) & (wind_speed_eff < this_rated_ws)
    rated_mask = (this_rated_ws <= wind_speed_eff) & (wind_speed_eff < this_turb_co)

    turb_pwr = tf.where(cut_in_mask,
                        this_rated_pwr * ((wind_speed_eff - this_turb_ci) / (this_rated_ws - this_turb_ci)) ** 3,
                        turb_pwr)
...
# @tf.function
# def fitness_tf_36(population: tf.Tensor) -> tf.Tensor:
#     """
#     Fitness function for the wind farm problem
#     :param population: Individual to evaluate
#     :type population: np.ndarray
...
#     """
#     population = tf.cast(population, dtype=tf.float32)
#     this_wind_speed = tf.constant(WindParameters36.speed, dtype=tf.float32)
#     this_turb_diam = tf.constant(TurbineParameters.diam, dtype=tf.float32)
#     this_turb_ci = tf.constant(TurbineParameters.ci, dtype=tf.float32)
#     this_turb_co = tf.constant(TurbineParameters.co, dtype=tf.float32)
#     this_rated_ws = tf.constant(TurbineParameters.rated_ws, dtype=tf.float32)
...
#     hrs_per_year = tf.constant(365. * 24. / 1.E6, dtype=tf.float32)
#
#     wind_dir_rad = tf.constant(WindParameters36.dirs_rad, dtype=tf.float32)
#
#     cos_dir = tf.cast(tf.cos(-wind_dir_rad), dtype=tf.float32)
#     sin_dir = tf.cast(tf.sin(-wind_dir_rad), dtype=tf.float32)
#
...
#
#     pwr_produced = tf.reduce_sum(turb_pwr, axis=-1)
#     aep = hrs_per_year * (WindParameters36.freq * pwr_produced)
#     return aep
#
#
# @tf.function
# def fitness_tf_64(population: tf.Tensor) -> tf.Tensor:
#     """
#     Fitness function for the wind farm problem
#     :param population: Individual to evaluate
#     :type population: np.ndarray
...
#     """
#     population = tf.cast(population, dtype=tf.float32)
#     this_wind_speed = tf.constant(WindParameters64.speed, dtype=tf.float32)
#     this_turb_diam = tf.constant(TurbineParameters.diam, dtype=tf.float32)
#     this_turb_ci = tf.constant(TurbineParameters.ci, dtype=tf.float32)
#     this_turb_co = tf.constant(TurbineParameters.co, dtype=tf.float32)
#     this_rated_ws = tf.constant(TurbineParameters.rated_ws, dtype=tf.float32)
...
#     hrs_per_year = tf.constant(365. * 24. / 1.E6, dtype=tf.float32)
#
#     wind_dir_rad = tf.constant(WindParameters64.dirs_rad, dtype=tf.float32)
#
#     cos_dir = tf.cast(tf.cos(-wind_dir_rad), dtype=tf.float32)
#     sin_dir = tf.cast(tf.sin(-wind_dir_rad), dtype=tf.float32)
#
...
#
#     pwr_produced = tf.reduce_sum(turb_pwr, axis=-1)
#     aep = hrs_per_year * (WindParameters16.freq * pwr_produced)
#     return aep
#
#
