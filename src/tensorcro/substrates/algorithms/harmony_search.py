...
class HarmonySearch(CROSubstrate):
    ...
    def _call(self, individuals: tf.Tensor):
        ...
        tiled = tf.tile([tf.range(np)], [noi, 1])
        indices = tf.random.uniform((noi, np), minval=0, maxval=noi, dtype=tf.int32)
        all_indices = tf.concat([tf.expand_dims(indices, -1), tf.expand_dims(tiled, -1)], axis=-1)
        cross_individuals = tf.gather_nd(individuals, all_indices)

        crossover_individuals = tf.where(binary_mask_0, cross_individuals, random_individuals)
        uniforms = tf.add(crossover_individuals, self.bw * self.mut(crossover_individuals))
        return tf.where(binary_mask_1, crossover_individuals, uniforms)


class HarmonyMutation(CROSubstrate):
    def __init__(self, directives: tf.Tensor, hmc_r: float = 0.95, pa_r: float = 0.8, bandwidth: float = 0.2):
        """
        Harmony Search Crossover Operator implements the Harmony Search algorithm, but there is no crossover between
        harmonies.
        It is a crossover operator that uses
        the following formula:
        ...
        .. math::
            x_i^{(k+1)} =
                ...
                x_i^{(k)} + BW \cdot (U_{min} - U_{max}) & if  r_1 >= HMC_{r}  and  r_2 \geq PA_{r}
        ..
        Where :math:`BW` is the band-width, :math:`HMC_{r}` is the Harmony Memory
        Consideration rate, :math:`PA_{r}` is
        the Pitch Adjustment rate and :math:`r_1` and :math:`r_2` are random numbers between 0 and 1.

        :param directives: Parameter specifications.
        :param hmc_r: The Harmony Memory Consideration rate for ONE INDIVIDUAL(HMCR).
        :param pa_r: The Pitch Adjustment rate (PAR).
        :param bandwidth: The band-width (BW).
        """
        if not isinstance(directives, tf.Tensor):
    ...
    def _call(self, individuals: tf.Tensor):
        ...
        crossover_individuals = tf.where(binary_mask_0, individuals, random_individuals)
